...
<template>
  <div class="datetime-picker" :style="{ width: width }">
    <input :placeholder="placeholder" type="text" :name="name" :style='styleObj' :readonly="readonly" :value="showDate" @click="show = !show">
    <div v-show="show" :class="['picker-wrap', isRange ? 'picker-wrap-range' : '']">
      <table class="date-picker" v-for="(date, no) in dateList">
        ...
        <tbody>
          <tr v-for="i in 6">
            <td v-for="j in 7" :class="dateList[no][(i - 1) * 7 + j - 1] && dateList[no][(i - 1) * 7 + j - 1].status" :date="dateList[no][(i - 1) * 7 + j - 1] && dateList[no][(i - 1) * 7 + j - 1].date" @click="pickDate((i - 1) * 7 + j - 1, no)">{{dateList[no][(i - 1) * 7 + j - 1] && dateList[no][(i - 1) * 7 + j - 1].text}}</td>
          </tr>
        </tbody>
        <tfoot v-if="!isRange">
          <tr>
      ...
      pickDate (i, index) {
        ...
        if (this.isRange) {
          if (this.value.length === 2) {
            ...
            // this.$set(this.now, 1, new Date(this.dateList[index][i].time))
            this.$set(this.dateArea, 1, new Date(this.dateList[index][i].time))
            this.value.push(this.stringify(this.dateList[index][i].time, 'YYYY-MM-DD'))
            this.myclick && this.myclick()
            this.show = false
          } else {
            // this.$set(this.now, 0, new Date(this.dateList[index][i].time))
            this.$set(this.dateArea, 0, new Date(this.dateList[index][i].time))
            this.value.push(this.stringify(this.dateList[index][i].time, 'YYYY-MM-DD'))
          }
        // 日期选择
        } else {
